...
class WC_Gateway_Kitegateway extends WC_Payment_Gateway {
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'api_key'             => array(
                'title'       => __( 'API Key', 'kitegateway-payments-woo' ),
                'type'        => 'text',
                'description' => __( 'Add your API key', 'kitegateway-payments-woo' ),
                'desc_tip'    => true,
            ),
            'api_secret'             => array(
                'title'       => __( 'API Secret', 'kitegateway-payments-woo' ),
                'type'        => 'text',
                'description' => __( 'Add your API Secret', 'kitegateway-payments-woo' ),
                'desc_tip'    => true,
            ),
            'description'        => array(
            ...
            'enable_for_methods' => array(
                'title'             => __( 'Enable for shipping methods', 'kitegateway-payments-woo' ),
                'type'              => 'multiselect',
                'class'             => 'wc-enhanced-select',
                'css'               => 'width: 400px;',
                'default'           => '',
                'description'       => __( 'If kitegateway is only available for certain methods, set it up here. Leave blank to enable for all methods.', 'kitegateway-payments-woo' ),
                'options'           => $this->load_shipping_method_options(),
                'desc_tip'          => true,
                'custom_attributes' => array(
                    'data-placeholder' => __( 'Select shipping methods', 'kitegateway-payments-woo' ),
                ),
            ),
            'enable_for_virtual' => array(
                'title'   => __( 'Accept for virtual orders', 'kitegateway-payments-woo' ),
                'label'   => __( 'Accept kitegateway if the order is virtual', 'kitegateway-payments-woo' ),
                'type'    => 'checkbox',
                'default' => 'yes',
            ),
        );
    ...
    private function check_openssl_for_settings() {
        ...
        // Log OpenSSL version
        error_log('Kitegateway OpenSSL version: ' . OPENSSL_VERSION_TEXT);
    ...
    private function send_admin_notice($message, $send_email = false) {
        ...
        error_log('Kitegateway Admin Notice: ' . $message);
    ...
    private function load_shipping_method_options() {
        ...
        foreach ( WC()->shipping()->load_shipping_methods() as $method ) {
            ...
            $options[ $method->get_method_title() ][ $method->id ] = sprintf( __( 'Any "%1$s" method', 'kitegateway-payments-woo' ), $method->get_method_title() );
            ...
            foreach ( $zones as $zone ) {
                ...
                foreach ( $shipping_method_instances as $shipping_method_instance_id => $shipping_method_instance ) {
                    ...
                    $option_instance_title = sprintf( __( '%1$s (#%2$s)', 'kitegateway-payments-woo' ), $shipping_method_instance->get_title(), $shipping_method_instance_id );

                    // Translators: %1$s zone name, %2$s shipping method instance name.
                    $option_title = sprintf( __( '%1$s – %2$s', 'kitegateway-payments-woo' ), $zone->get_id() ? $zone->get_zone_name() : __( 'Other locations', 'kitegateway-payments-woo' ), $option_instance_title );
    ...
    private function kitegateway_payment_processing( $order ) {
        ...
        if ( is_wp_error( $auth_response ) ) {
            ...
            wc_get_logger()->error( 'Kitegateway Payment Error: ' . $auth_error_message, array( 'source' => 'kitegateway-payments-woo' ) );

            return "Something went wrong with auth: $auth_error_message";
        }

        if ( 200 !== wp_remote_retrieve_response_code( $auth_response ) ) {
            // $order->update_status( apply_filters( 'woocommerce_kitegateway_process_payment_order_status', $order->has_downloadable_item() ? 'wc-invoiced' : 'processing', $order ), __( 'Payments pending.', 'kitegateway-payments-woo' ) );
        }
        ...
        if ( 200 === wp_remote_retrieve_response_code( $auth_response ) ) {
            ...
            // Log card data size and sanitized structure
            error_log('Kitegateway Card data size: ' . strlen($card_data) . ' bytes');
            error_log('Kitegateway Card data (sanitized): ' . json_encode(array(
                'full_name_length' => strlen($order->get_billing_first_name() . ' ' . $order->get_billing_last_name()),
                'card_number_length' => strlen($card_number),
                'expiry_month_length' => strlen($expiry_month),
                'expiry_year_length' => strlen($expiry_year),
                'cvv_length' => strlen($cvv),
                'billing_address_length' => strlen($order->get_billing_address_1()),
                'billing_city_length' => strlen($order->get_billing_city()),
                'billing_zip' => 'N/A',
                'billing_state_length' => strlen($order->get_billing_state()),
                'billing_country_length' => strlen($order->get_billing_country())
            )));
            ...
            if ( is_wp_error( $collection_response ) ) {
                ...
                wc_get_logger()->error( 'Kitegateway Payment Error: ' . $collection_error_message, array( 'source' => 'kitegateway-payments-woo' ) );
            ...
            if ( $response_code !== 200 && $response_code !== 202 ) {
                ...
                wc_add_notice(__($message, 'docs.kitegateway.com'), 'error');

                return array(
                    'result'   => 'failure',
                    'messages' => __($message, 'docs.kitegateway.com'),
                    'code'     => $code,
                );
            }
    ...
    public function thankyou_page($id) {
        ...
        if (empty($query_params['status'])) {
            wc_get_logger()->debug('Kitegateway Thankyou: Missing payment status in query parameters', array('source' => 'kitegateway-payments-woo'));
            return;
        }
        ...
        wc_get_logger()->debug('Kitegateway Thankyou: Order ID from query parameters: ' . $order_id, array('source' => 'kitegateway-payments-woo'));

         wc_get_logger()->debug('Kitegateway Thankyou: ID from query parameters: ' . $id, array('source' => 'kitegateway-payments-woo'));

        // Verify order_id and key
        if ($order_id != $id) {
            wc_get_logger()->error('Kitegateway Thankyou Error: Invalid order_id or key', array('source' => 'kitegateway-payments-woo'));
            wc_add_notice(__('Invalid payment confirmation data.', 'kitegateway-payments-woo'), 'error');
            return;
        }
        ...
        // $keyContent = file_get_contents($file);
        // $publicKey = openssl_get_publickey($keyContent);
        // $strPayload =  $query_params['transaction_id'].":".$query_params['merchant_reference'].":".$query_params['kitegateway_reference'].":".$query_params['status'].":".$this->get_return_url( $order );
    
        // if(openssl_verify($strPayload, $query_params['signature'], $publicKey, OPENSSL_ALGO_SHA512)) {
        //     wc_get_logger()->debug('Kitegateway redirect: Signature verified successfully', array('source' => 'kitegateway-payments-woo'));
        // } else {
        //     wc_get_logger()->error('Kitegateway redirect Error: Invalid signature', array('source' => 'kitegateway-payments-woo'));
        //     wc_get_logger()->debug('Kitegateway redirect signature: ' . print_r($query_params['signature'], true), array('source' => 'kitegateway-payments-woo'));
        //     wc_get_logger()->debug('Kitegateway redirect query paramters: ' . print_r($strPayload, true), array('source' => 'kitegateway-payments-woo'));
        //     wc_add_notice(__('Invalid redirect signature', 'kitegateway-payments-woo'), 'error');
    ...
    public function handle_webhook() {
        ...
        $payload = json_decode($raw_data, true);
         ...
         // Log webhook payload
        wc_get_logger()->debug('Kitegateway Webhook Payload: ' . print_r($payload, true), array('source' => 'kitegateway-payments-woo'));
        ...
        if(openssl_verify($strPayload, $signature, $publicKey, OPENSSL_ALGO_SHA512)) {
            wc_get_logger()->debug('Kitegateway Webhook: Signature verified successfully', array('source' => 'kitegateway-payments-woo'));
        } else {
            wc_get_logger()->error('Kitegateway Webhook Error: Invalid signature', array('source' => 'kitegateway-payments-woo'));
            wc_get_logger()->debug('Kitegateway Webhook signature: ' . print_r($signature, true), array('source' => 'kitegateway-payments-woo'));
            wc_get_logger()->debug('Kitegateway Webhook Payload: ' . print_r($strPayload, true), array('source' => 'kitegateway-payments-woo'));
            status_header(401);
        ...
        if (!isset($order_id) || !isset($payload['transaction_status'])) {
            wc_get_logger()->error('Kitegateway Webhook Error: Invalid payload', array('source' => 'kitegateway-payments-woo'));
            status_header(400);
        ...
        if (!$order) {
            wc_get_logger()->error("Kitegateway $source Error: Order not found for ID $order_id", array('source' => 'kitegateway-payments-woo'));
            status_header(400);
        ...
        switch (strtolower($payment_status)) {
            ...
            case 'success':
                if ($order->get_status() !== 'completed') {
                    ...
                    $order->add_order_note(__("Payment completed via Kitegateway ($source). Transaction Reference: $kitegateway_reference", 'kitegateway-payments-woo'));
                    wc_get_logger()->debug("Kitegateway $source: Order $order_id marked as completed. Transaction Reference: $kitegateway_reference", array('source' => 'kitegateway-payments-woo'));
                }
                break;
            case 'failed':
            case 'FAILED':
            case 'FAIL':
                if (!$order->has_status(array('failed', 'cancelled'))) {
                    $order->update_status('failed', __("Payment failed via Kitegateway ($source).Transaction Reference: $kitegateway_reference. Message: $message", 'kitegateway-payments-woo'));
                    $order->add_order_note(__("Payment failed via Kitegateway ($source).Transaction Reference: $kitegateway_reference. Message: $message", 'kitegateway-payments-woo'));
                    wc_get_logger()->debug("Kitegateway $source: Order $order_id marked as failed", array('source' => 'kitegateway-payments-woo'));
                }
                break;
            case 'REFUNDED':
            case 'REFUND':
                if ($order->get_status() !== 'refunded') {
                    $order->update_status('refunded', __("Payment refunded via Kitegateway ($source).", 'kitegateway-payments-woo'));
                    $order->add_order_note(__("Payment refunded via Kitegateway ($source). Transaction Reference: $kitegateway_reference", 'kitegateway-payments-woo'));
                    wc_get_logger()->debug("Kitegateway $source: Order $order_id marked as refunded. Transaction Reference: $kitegateway_reference", array('source' => 'kitegateway-payments-woo'));
                }
                break;
            case 'PENDING':
                if (!$order->has_status(array('pending', 'on-hold'))) {
                    $order->update_status('on-hold', __("Payment pending via Kitegateway ($source).Transaction Reference: $kitegateway_reference", 'kitegateway-payments-woo'));
                    $order->add_order_note(__("Payment pending via Kitegateway ($source).", 'kitegateway-payments-woo'));
                    wc_get_logger()->debug("Kitegateway $source: Order $order_id marked as on-hold", array('source' => 'kitegateway-payments-woo'));
                }
                break;
            case 'PROCESSING':
                if (!$order->has_status(array('pending', 'on-hold'))) {
                ...
                break;    
            default:
                wc_get_logger()->error("Kitegateway $source Error: Unknown payment status: $payment_status for order $order_id", array('source' => 'kitegateway-payments-woo'));
                if ($source === 'Redirect') {
                    wc_add_notice(__("Unknown payment status: $payment_status", 'kitegateway-payments-woo'), 'error');
                }
                return;
        }
    }
    ...
    public function encrypt($card_string) {
        ...
        if (!extension_loaded('openssl')) {
            error_log('Kitegateway Encryption Error: OpenSSL extension is not enabled.');
            return $ret;
        }
        ...
        $keyContent = @file_get_contents($file);
        if ($keyContent === false) {
            error_log('Kitegateway Encryption Error: Unable to load public key file from ' . esc_url($file));
            return $ret;
        }
        ...
        $publicKey = openssl_get_publickey($keyContent);
        if ($publicKey === false) {
            error_log('Kitegateway Encryption Error: Invalid public key - ' . openssl_error_string());
            return $ret;
        }
        ...
        if ($keyDetails === false || !isset($keyDetails['bits']) || $keyDetails['bits'] !== 2048) {
            error_log('Kitegateway Encryption Error: Invalid key size. Expected 2048-bit, got ' . ($keyDetails['bits'] ?? 'unknown') . ' bits.');
            openssl_free_key($publicKey);
            return $ret;
        }
        error_log('Kitegateway Public key modulus (hex): ' . bin2hex($keyDetails['rsa']['n']));

        // Check data size (max 214 bytes for 2048-bit key with OAEP padding)
        $dataSize = strlen($card_string);
        error_log('Kitegateway Card data size: ' . $dataSize . ' bytes');
        ...
        if ($dataSize > 214) {
            ...
            if ($encrypted_data === false) {
                error_log('Kitegateway Encryption Error: AES encryption failed - ' . openssl_error_string());
                openssl_free_key($publicKey);
                return $ret;
            }

            if (!openssl_public_encrypt($aes_key, $encrypted_aes_key, $publicKey, OPENSSL_PKCS1_OAEP_PADDING)) {
                error_log('Kitegateway Encryption Error: RSA encryption of AES key failed - ' . openssl_error_string());
                openssl_free_key($publicKey);
                return $ret;
            }

            // Log component sizes
            error_log('Kitegateway AES encrypted data size: ' . strlen($encrypted_data) . ' bytes');
            error_log('Kitegateway IV size: ' . strlen($iv) . ' bytes');
            error_log('Kitegateway Encrypted AES key size: ' . strlen($encrypted_aes_key) . ' bytes');

            openssl_free_key($publicKey);
            $ret = base64_encode($encrypted_data . '::' . base64_encode($iv) . '::' . base64_encode($encrypted_aes_key));
            if (!preg_match('/^[A-Za-z0-9+\/=]+$/', $ret)) {
                error_log('Kitegateway Encryption Error: Invalid base64 encoding of hybrid ciphertext.');
                return null;
            }
            error_log('Kitegateway Hybrid encrypted data (base64): ' . $ret);
            return $ret;
        }
        ...
        if (!openssl_public_encrypt($card_string, $result, $publicKey, OPENSSL_PKCS1_OAEP_PADDING)) {
            error_log('Kitegateway Encryption Error: RSA encryption failed - ' . openssl_error_string());
            openssl_free_key($publicKey);
            return $ret;
        }
        ...
        if (!preg_match('/^[A-Za-z0-9+\/=]+$/', $ret)) {
            error_log('Kitegateway Encryption Error: Invalid base64 encoding of ciphertext.');
            return null;
        }
        error_log('Kitegateway Encrypted data (base64): ' . $ret . ', size: ' . strlen($result) . ' bytes');
